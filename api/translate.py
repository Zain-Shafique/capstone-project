...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if content_length == 0:
            ...
            log_request("/api/translate", {}, response, start_time)
            return
        ...
        if not is_valid:
            ...
            self._send_response(response)
            log_request("/api/translate", request_data, response, start_time)
            return
        ...
        try:
            ...
            if to_lang not in SUPPORTED_LANGUAGES:
                response = make_response(400, f"Target language '{to_lang}' is not supported. Available languages: {', '.join(SUPPORTED_LANGUAGES.keys())}")
                self._send_response(response)
                log_request("/api/translate", request_data, response, start_time)
                return
                
            result = translate_text(text, from_lang, to_lang)
            response = make_response(200, "Translation completed successfully", result)
